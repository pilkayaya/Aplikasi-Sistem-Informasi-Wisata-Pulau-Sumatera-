...
# from wisata.webservice import serializers
...
from webservice.serializers import (
    RegisterSerializer, LoginSerializer, KategoriProvinsiSerializer, KategoriObjekWisataSerializer, ObjekWisataSerializer, OperasionalSerializer, ReviewSerializer, ProfileSerializer, UserSerializer
)
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            profil_data = profile_serializer.data
...
class ObjekWisataListApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = ObjekWisataSerializer(objek_wisata, many=True, context={'request': request})
        response = {
...
class ObjekWisataFilter(generics.ListAPIView):
    ...
    search_fields = ['nama','kategori_provinsi__provinsi', 'kategori_objek_wisata__kategori']
...
class ReviewApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = {
            'user_profile_id': user_profile.pk,  # Menggunakan primary key dari user_profile
            'objek_wisata': request.data.get('objek_wisata'),
        ...
        if serializer.is_valid():
            serializer.save(user_create=request.user)
            response = {
                'status': status.HTTP_201_CREATED,
                'message': 'Data created successfully...',
                'data': serializer.data
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save(user_update=request.user)
            response = {
                'status': status.HTTP_200_OK,
                'message': 'Data updated successfully...',
                'data': serializer.data
...
class ProfileApiView(APIView):
    ...
    def get(self, request, id=None, *args, **kwargs):
        if id:
            profile_instance = self.get_object(id)
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        user_serializer = UserSerializer(instance=profile_instance.user, data=user_data, partial=True)
